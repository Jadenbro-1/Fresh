...
import { View, Text, TouchableOpacity, StyleSheet, Dimensions, FlatList, Image, Animated, ScrollView, PanResponder, ActivityIndicator } from 'react-native';
  ...
  const fetchVideos = async () => {
    ...
    try {
      ...
      const mappedVideos = data.map((video, index) => ({
        ...
        videoUrl: { uri: video.url.startsWith('http') ? video.url.replace('http://', 'https://') : video.url },
        username: `@${video.author_first_name}-${video.author_last_name}`,
        profilePic: ('/Users/jadenbrozynski/fresh/assets/profilepic.jpg'),
  ...
  const renderVideoItem = ({ item, index }) => {
    ...
    return (
      <View style={styles.videoContainer} {...panResponder.panHandlers}>
        <TouchableOpacity activeOpacity={1} onPress={() => togglePause(index)} style={{ flex: 1 }}>
          {item.videoUrl ? (
            <Video
          ...
          <Animated.View style={[styles.videoDetailsContainer]}>
            ...
            {showComments[index] && (
              <View style={styles.commentsSection}>
                ...
                <TouchableOpacity style={styles.closeCommentsButton} onPress={() => toggleComments(index)}>
  ...
  return (
    <View style={styles.background}>
      <View style={styles.topBar}>
        <TouchableOpacity style={styles.topBarButton} onPress={() => navigation.navigate('YourPicks')}>
          <Text style={styles.topBarButtonText}>Your Picks</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.topBarButton} onPress={() => navigation.navigate('Explore')}>
